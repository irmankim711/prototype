...
const StyledTableHead = styled("tr")<{ primaryColor: string }>(
  ({ primaryColor }) => ({
    backgroundColor: primaryColor,
  })
);

const StyledTableHeader = styled("th")<{ bordered: boolean }>(
  ({ theme, bordered }) => ({
...
const StyledTableRow = styled("tr")<{ striped: boolean; index: number }>(
  ({ striped, index }) => ({
    backgroundColor: striped && index % 2 === 1 ? "#f9fafb" : "transparent",
  })
);

const StyledTableCell = styled("td")<{ bordered: boolean }>(
  ({ theme, bordered }) => ({
    padding: theme.spacing(1.5),
    border: bordered ? "1px solid #e5e7eb" : "none",
...
const ModernReportBuilder: React.FC = () => {
  ...
  const renderHeading = useCallback(
    ...
    [activeSection, previewMode, reportData.theme.fontFamily, deleteSection]
  );
  ...
  const renderChart = useCallback(
    ...
    [activeSection, previewMode, reportData.theme.primaryColor, deleteSection]
  );
  ...
  const renderTable = useCallback(
      ...
      return (
        ...
        >
          <Box sx={{ ...section.style, overflow: "auto" }}>
            <StyledTable>
              <thead>
                <StyledTableHead primaryColor={reportData.theme.primaryColor}>
                  {tableData.headers.map((header, index) => (
                    <StyledTableHeader
                      key={index}
                      bordered={tableData.bordered || false}
                    >
                      {header}
                    </StyledTableHeader>
                  ))}
              ...
              <tbody>
                {tableData.rows.map((row, rowIndex) => (
                  <StyledTableRow
                    key={rowIndex}
                    striped={tableData.striped || false}
                    index={rowIndex}
                  >
    ...
    [activeSection, previewMode, reportData.theme, deleteSection]
  );
  ...
  const renderList = useCallback(
    ...
    [activeSection, previewMode, deleteSection]
  );
  ...
  // Helper functions
  const getSectionDefaults = useCallback((type: Section["type"]) => {
    switch (type) {
      case "heading":
        return "New Heading";
      case "paragraph":
  ...
  const getDefaultStyles = useCallback(
    (type: Section["type"]): SectionStyle => {
      switch (type) {
        case "heading":
          return {
            fontSize: "28px",
  ...
  const addSection = useCallback(
    ...
    [reportData.sections.length, getSectionDefaults, getDefaultStyles]
  );
              ...
              {activeSection && (
                <Box sx={{ pt: 3, borderTop: 1, borderColor: "divider" }}>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      console.log("Style editor not implemented yet")
                    }
                  >
                    Style Options
                  </Button>
