...
const TemplateManager: React.FC = () => {
  ...
  const loadTemplates = () => {
    ...
    const defaultTemplates: Template[] = [
      {
        id: '1',
        name: 'Laporan FU Puncak Alam',
        type: 'docx',
        description: 'Official Puncak Alam program report template with participant tracking',
        content: `% Template content managed in backend
...
Template Fields:
- {{ NAMA_PESERTA_HADIR }}: Participant name
- {{ MARKAH_PRE }}: Pre-test score
- {{ KAD_PENGENALAN }}: IC number
- {{ ALAMAT }}: Address
- {{ NO_TELEFON }}: Phone number
- {{ KEHADIRAH_SABTU }}: Saturday attendance
- {{ KEHADIRAN_AHAD }}: Sunday attendance
- {{ ALASAN }}: Reason/notes
`,
