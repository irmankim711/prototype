...
export const AnalyticsDashboard: React.FC = () => {
    ...
    try {
      ...
      setRetryCounts(prev => {
        if (prev.topForms < 2 && !isRetry) {
          ...
          return { ...prev, topForms: prev.topForms + 1 };
        }
        return prev;
      });
    } finally {
    ...
    try {
      ...
      setRetryCounts(prev => {
        if (prev.performance < 2 && !isRetry) {
          ...
          return { ...prev, performance: prev.performance + 1 };
        }
        return prev;
      });
    } finally {
    ...
    try {
      ...
      setRetryCounts(prev => {
        if (prev.geographic < 2 && !isRetry) {
          ...
          return { ...prev, geographic: prev.geographic + 1 };
        }
        return prev;
      });
    } finally {
  ...
  const fetchFieldAnalytics = useCallback(async (formId: number, isRetry: boolean = false) => {
    ...
    try {
      ...
      // Auto-retry logic with cleanup tracking
      setRetryCounts(prev => {
        if (prev.fieldAnalytics < 2 && !isRetry) {
          const timeoutId = setTimeout(() => {
            if (isMountedRef.current) {
          ...
          return { ...prev, fieldAnalytics: prev.fieldAnalytics + 1 };
        }
        return prev;
      });
    } finally {
