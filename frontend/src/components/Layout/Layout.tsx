...
const MenuItem = styled(ListItemButton, { shouldForwardProp: (prop) => prop !== 'selected' })((props) => ({
  margin: '4px 12px',
  borderRadius: '8px',
...
interface MenuItem {
  ...
  path?: string;
  onClick?: (e: React.MouseEvent, navigate: Function) => void;
  submenu?: MenuItem[];
  badge?: number;
}
...
const menuItems: MenuItem[] = [
  ...
  { 
    ...
    submenu: [
      { text: 'Generate Report', icon: <GenerateReportIcon />, path: '/reports/new' },
      { text: 'Report Templates', icon: <FileIcon />, path: '/reports/templates' },
      { text: 'Previous Reports', icon: <DescriptionIcon />, path: '/reports/previous' },
    ]
  },
  { text: 'Submissions', icon: <SubmissionIcon />, path: '/submission', badge: 3 },
            ...
            <Box key={item.text}>
              <MenuItem
                selected={item.submenu 
                  ? item.submenu.some(subItem => location.pathname === subItem.path) || expandedSubmenu === item.text
              ...
              >
                ...
                {item.submenu && (
                  expandedSubmenu === item.text ? <ExpandLessIcon /> : <ExpandMoreIcon />
                )}
              </MenuItem>
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      >
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div" sx={{ fontFamily: '"Poppins", sans-serif', fontWeight: 500 }}>
            {menuItems.find((item) => item.path === location.pathname)?.text || 'StratoSys Report'}
          </Typography>
          <Box sx={{ flexGrow: 1 }} />
          <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <Tooltip title="Notifications">
              ...
              >
                <Badge badgeContent={3} color="error">
                  <NotificationsIcon sx={{ fontSize: '20px', color: '#5f6368' }} />
                </Badge>
              </IconButton>
            </Tooltip>
            ...
            >
              <Typography sx={{ p: 2, fontWeight: 500, color: '#0e1c40' }}>Help Resources</Typography>
              <Divider />
            ...
            >
              <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography sx={{ fontWeight: 500, color: '#0e1c40' }}>Notifications</Typography>
                <Button size="small" sx={{ color: '#4a69dd', textTransform: 'none' }}>
                  Mark all as read
                </Button>
              </Box>
              <Divider />
              {[
                { title: 'New report submitted', time: '2 hours ago', read: false },
                { title: 'System update completed', time: 'Yesterday', read: true },
                { title: 'Your report has been approved', time: '3 days ago', read: true }
              ].map((notification, index) => (
                <Box key={index}>
                  <MuiMenuItem onClick={handleNotificationsClose} sx={{ py: 2 }}>
                    <Box sx={{ 
                      width: '8px', 
                      height: '8px', 
                      borderRadius: '50%', 
            ...
            >
              <Box sx={{ px: 2, py: 1.5 }}>
                ...
                <Typography variant="body2" sx={{ color: '#64748b' }}>john.doe@example.com</Typography>
              </Box>
              <Divider />
