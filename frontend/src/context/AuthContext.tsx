...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const api = axios.create({ baseURL: import.meta.env.VITE_AUTH_API_URL || 'http://localhost:5000/api', withCredentials: true });
  ...
  const hasRefreshToken = () => {
    ...
    return localStorage.getItem('accessToken') !== null || document.cookie.includes('refresh_token');
  };
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        console.log('Invalid token in localStorage, clearing and trying refresh');
        localStorage.removeItem('accessToken');
        if (hasRefreshToken()) {
          refreshTokenSilently();
        }
