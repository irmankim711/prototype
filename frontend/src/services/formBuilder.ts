...
export const formBuilderAPI = {
  ...
  // Update a form
  updateForm: async (formId: number, formData: Partial<{
    title: string;
    description: string;
    schema: FormSchema;
    is_active: boolean;
  ...
  submitForm: async (formId: number, data: {
    data: Record<string, any>;
    email?: string;
  }) => {
  ...
  updateSubmissionStatus: async (submissionId: number, status: string) => {
    const response = await api.put(`/api/forms/submissions/${submissionId}/status`, {
      status
    });
    return response.data;
  }
...
export const formBuilderUtils = {
  ...
  validateFormData: (schema: FormSchema, data: Record<string, any>): string[] => {
    const errors: string[] = [];
  ...
  // Duplicate a form
  duplicateForm: (form: Form): Omit<Form, 'id' | 'created_at' | 'updated_at'> => {
...
export const FORM_BUILDER_CONSTANTS = {
  DEFAULT_FIELD_TYPES: [
    ...
    'date', 'time', 'datetime', 'file', 'phone', 'url', 'rating', 'location'
  ],
