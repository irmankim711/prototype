...
const NextGenReportBuilderPage: React.FC = () => {
  ...
  const handleSaveReport = async (reportConfig: Partial<ReportConfig>) => {
    try {
      ...
      const savedReport = await nextGenReportService.saveReport(reportToSave);
      ...
      // Return the saved report so it can be used immediately
      return savedReport;
    } catch (error: any) {
  ...
  const handleExportReport = async (format: 'pdf' | 'excel' | 'powerpoint' | 'html', reportId?: string | number) => {
    try {
      ...
      const blob = await nextGenReportService.exportReport(idToUse, format);
      ...
      // Return the report ID for preview
      return idToUse;
    } catch (error) {
  ...
  // Handle Excel-based report generation
  const handleGenerateReportFromExcel = async (excelFilePath: string, templateId: string, reportTitle: string): Promise<any> => {
    try {
      setLoading(true);
      setError(null);

      console.log('🎯 NextGenReportBuilderPage: Generating report from Excel:', {
        excelFilePath,
        templateId,
        reportTitle
      });

      const generatedReport = await nextGenReportService.generateReportFromExcel(
        excelFilePath,
        templateId,
        reportTitle
      );
      ...
      if (generatedReport) {
        ...
        // Normalize the report object to ensure consistent ID access
        ...
        setSuccess('Report generated successfully from Excel!');
        ...
        throw new Error('No report returned from Excel generation');
      }
    } catch (error: any) {
      console.error('Error generating report from Excel:', error);

      const errorMessage = error.message || 'Failed to generate report from Excel';
      setError(errorMessage);
      ...
      <AuthGuard>
        <Box sx={{ height: "100vh", overflow: "hidden" }}>
          ...
          <NextGenReportBuilder
            ...
            onGenerateReportFromExcel={handleGenerateReportFromExcel}
