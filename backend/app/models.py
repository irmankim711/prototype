...
class Report(db.Model):
    ...
    def update_status(self, status: str, error_message: str = None, processing_notes: str = None):
        """Update report generation status using actual database fields"""
        ...
        # Set generated_at timestamp when completed
        if status == 'completed' and not self.generated_at:
            self.generated_at = datetime.utcnow()

        # Calculate generation time if we have both timestamps
        if status == 'completed' and self.created_at and self.generated_at:
