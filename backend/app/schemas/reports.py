...
class ReportCreateRequest(BaseRequestSchema):
    ...
    priority: str = Field(default="medium", pattern="^(low|medium|high|urgent)$", description="Report priority")
    ...
    @classmethod
    def validate_data_source(cls, v: Dict[str, Any]) -> Dict[str, Any]:
...
class ReportUpdateRequest(BaseRequestSchema):
    ...
    priority: Optional[str] = Field(default=None, pattern="^(low|medium|high|urgent)$", description="Report priority")
...
class ReportExportRequest(BaseRequestSchema):
    ...
    # Quality settings
    quality: str = Field(default="standard", pattern="^(draft|standard|high|print)$", description="Export quality")
...
class AIAnalysisRequest(BaseRequestSchema):
    ...
    analysis_type: str = Field(..., pattern="^(insights|anomalies|trends|recommendations|summary)$", description="Type of analysis")
    language: str = Field(default="en", pattern="^[a-z]{2}$", description="Analysis language")
    ...
    # Output preferences
    detail_level: str = Field(default="standard", pattern="^(basic|standard|detailed|comprehensive)$", description="Analysis detail level")
    ...
    @classmethod
    def validate_creativity_level(cls, v: float) -> float:
...
class ReportSearchRequest(BaseRequestSchema):
    ...
    def validate_date_ranges(cls, values):
        ...
        if hasattr(values, 'created_after') and hasattr(values, 'created_before'):
            if values.created_after and values.created_before:
                if values.created_after >= values.created_before:
                    raise ValueError('created_after must be before created_before')
        if hasattr(values, 'updated_after') and hasattr(values, 'updated_before'):
