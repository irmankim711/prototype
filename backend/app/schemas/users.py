...
class UserCreateRequest(BaseRequestSchema):
    ...
    def validate_password_confirmation(cls, values):
        ...
        if hasattr(values, 'password') and hasattr(values, 'confirm_password'):
...
class UserUpdateRequest(BaseRequestSchema):
    ...
    theme: Optional[str] = Field(default=None, pattern="^(light|dark|auto)$", description="UI theme preference")
...
class UserPasswordChangeRequest(BaseRequestSchema):
    ...
    def validate_password_confirmation(cls, values):
        """Validate password confirmation matches."""
        if hasattr(values, 'new_password') and hasattr(values, 'confirm_new_password'):
...
class UserSearchRequest(BaseRequestSchema):
    ...
    def validate_date_ranges(cls, values):
        ...
        if hasattr(values, 'last_login_after') and hasattr(values, 'last_login_before'):
