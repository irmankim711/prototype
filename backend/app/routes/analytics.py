...
def validate_firebase_user():
    """Validate Firebase authentication and extract user identity with detailed logging"""
    try:
        current_user_id = get_current_user_id()

        logger.info(f"✅ Firebase validation successful for user ID: {current_user_id}")
        ...
        if current_user_id is None:
            log_validation_error(
                ...
                'Firebase token is valid but contains no user identity',
        ...
        error_msg = f"Firebase validation failed: {str(e)}"
        log_validation_error(
            'Firebase_Validation',
            'firebase_verification_failed',
            error_msg,
            received={'error': str(e)},
            expected={'valid_firebase_token': 'Bearer <firebase_token>'}
        )
...
def get_analytics_dashboard_stats():
    ...
    try:
        # Validate Firebase auth and get user ID
        user_id, error = validate_firebase_user()
        if error:
