...
class Report(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            'id': self.id,
            'uuid': str(self.id),  # Add uuid field for compatibility
            ...
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.created_at.isoformat() if self.created_at else None,  # Add updated_at for compatibility
            # Add individual file path fields that the frontend expects
            'pdf_file_path': self.file_path if self.file_format == 'pdf' else None,
            'docx_file_path': self.file_path if self.file_format == 'docx' else None,
            'excel_file_path': self.file_path if self.file_format == 'excel' else None,
            'pdf_file_size': self.file_size if self.file_format == 'pdf' else None,
            'docx_file_size': self.file_size if self.file_format == 'docx' else None,
            'excel_file_size': self.file_size if self.file_format == 'excel' else None,
        }
