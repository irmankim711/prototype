...
def create_app(config_name=None):
    ...
    app.config['JWT_ACCESS_TOKEN_EXPIRES'] = timedelta(seconds=settings.security.jwt_access_token_expires)
    app.config['JWT_REFRESH_TOKEN_EXPIRES'] = timedelta(seconds=settings.security.jwt_refresh_token_expires)
    ...
    def user_identity_lookup(user):
        ...
        try:
            ...
            if isinstance(user, str):
                # If it's already a string, return as-is (allow any string identity)
                return user
    ...
    def user_lookup_callback(_jwt_header, jwt_data):
        ...
        try:
            # TODO: Implement proper admin user setup and management
            # For production, look up user in database
