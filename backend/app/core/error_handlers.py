...
def handle_rate_limit_exceeded(error: RateLimitExceeded) -> Tuple[Response, int]:
    """Handle Flask-Limiter rate limit exceeded exceptions."""
    try:
        request = request if 'request' in globals() else None
    except:
        request = None
    ...
    response = jsonify({
        'error': {
            ...
            'limit': str(getattr(error, 'limit', 'unknown'))
        },
        'success': False
    })
